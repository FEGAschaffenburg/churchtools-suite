...
// Plugin constants
define( 'CTS_ELEMENTOR_VERSION', '0.6.22' ); // Updated version for cache busting
...
add_action( 'plugins_loaded', function() {
	...
	// 1. Check if Repository Factory is available (Main Plugin >= v1.0.9.0)
	// This check must run first because the main plugin may initialise its main class later (on `init`).
	if ( ! function_exists( 'churchtools_suite_get_repository' ) ) {
		// Repository factory not available -> main plugin missing or too old
		add_action( 'admin_notices', 'cts_elementor_outdated_main_plugin_notice' );
		return;
	}

	// 2. If the main plugin class is not yet available, we do NOT bail out here.
	// The main plugin initialises later (fires `churchtools_suite_loaded`), so we register
	// our init to that hook below. Avoid showing a premature "missing main plugin" notice.
