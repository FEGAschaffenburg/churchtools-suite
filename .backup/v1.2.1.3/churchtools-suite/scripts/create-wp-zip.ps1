...
$outputBase = if (Test-Path "C:\privat") { "C:\privat" } else { Join-Path $RepoRoot "dist" }
if (-not (Test-Path $outputBase)) {
    New-Item -ItemType Directory -Path $outputBase -Force | Out-Null
}
$ArchiveDir = Join-Path $outputBase "archiv"
...
$OutputZip = Join-Path $outputBase "$PluginSlug-$Version.zip"
...
$oldZips = Get-ChildItem -Path $outputBase -Filter "$PluginSlug-*.zip" -ErrorAction SilentlyContinue
if ($oldZips) {
